...
render_point_cloud：这个函数就像是一个3D摄像机，它能在指定位置拍摄3D模型（点云）的照片，形成2D的渲染图。
paste_render_to_original_image：这个函数的作用就像是把你用上面那个"3D摄像机"拍摄的照片，贴到一个已经存在的照片/图像上面。
                                例如，把一个3D物体的渲染图像嵌入到一张真实的背景照片中。
get_sequence_pointcloud：这个函数的作用就像是一个3D扫描仪，它可以从一序列的图像中，提取出场景的主要物体形成一个3D模型（点云）。
get_eval_frame_data_pointcloud：这个函数也是类似3D扫描的功能，它从一个给定的数据帧集合中生成3D模型（点云），而生成的方式则是通过反向的深度投影。
_subsample_pointcloud：这个函数可以类比成是用一个筛子从大量的点云数据中随机选取部分点，以确保点云数据的处理在可控的范围内，同时也节省处理的时间。
 """
